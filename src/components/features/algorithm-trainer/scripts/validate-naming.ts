  ...
  if (fileName === "index.ts") {
    ...
    if (keyMatches) {
      keyMatches.forEach((match: string) => {
        ...
        // Check if key exists in any of the pattern objects
        const patternExists =
          key === monsterHunterPattern.title ||
          key === monsterHunterTestDataPattern.title ||
          key === monsterHunterGuidePattern.title;
        if (!patternExists) {
          issues.push(`Pattern key "${key}" not found in any pattern object`);
        }
      });
    }
  }
