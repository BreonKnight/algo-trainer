...
filePatternKeys.forEach((key) => {
  ...
  if (!patternObjKeys.includes(key)) {
    ...
    // You may want to auto-insert here if desired
  }
  // 2. PATTERN_KEYS
  ...
  if (!mhPatternKeys.includes(key)) {
    ...
    if (DRYRUN) {
      console.log(`[DRYRUN] Would append to ${mhPatternPath}:\n${content}`);
    } else {
  ...
  if (!mhGuideKeys.includes(key)) {
    ...
    if (DRYRUN) {
      console.log(`[DRYRUN] Would append to ${mhGuidePath}:\n${content}`);
    } else {
  ...
  if (!mhTestDataKeys.includes(key)) {
    ...
    if (DRYRUN) {
      console.log(`[DRYRUN] Would append to ${mhTestDataPath}:\n${content}`);
    } else {
      fs.appendFileSync(mhTestDataPath, content);
    }
  }
  // 6. Implementation file in grouped folder
  const group = groupForPattern(key);
  const implDir = path.join(implRoot, group);
  if (!fs.existsSync(implDir)) {
    if (DRYRUN) {
      console.log(`[DRYRUN] Would create directory ${implDir}`);
    } else {
      fs.mkdirSync(implDir, { recursive: true });
    }
  }
  const implFile = path.join(implDir, `${key.replace(/ /g, "")}.tsx`);
  if (!fs.existsSync(implFile)) {
    ...
    if (DRYRUN) {
      console.log(`[DRYRUN] Would create file ${implFile}`);
    } else {
