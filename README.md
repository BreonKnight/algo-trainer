...
- 📚 Built-in pseudocode patterns for common algorithms:
  ...
  | 🧩 Techniques      | `Dynamic Programming` `Greedy` `Backtracking` `Two Pointers` `Sliding Window` `Bit Manipulation` `State Compression DP` `Digit DP` `Tree (Dynamic Programming)` `Probability DP` `Bitwise DP`                                                                                                                                                                                                                            |
  ...
  | 📝 Advanced        | `Monotonic Stack` `Monotonic Queue` `Bit Manipulation` `Topological Sort` `A* Search` `Dijkstra` `Kosaraju's Algorithm` `Articulation Points` `Bridges` `Strongly Connected Components` `Network Flow` `Maximum Bipartite Matching` `Grid Traversal` `Kruskal's Algorithm` `Prim's Algorithm` `Bellman-Ford` `Floyd-Warshall Algorithm` `Ford-Fulkerson Algorithm` `Hopcroft-Karp's Algorithm` `Kahn's Topological Sort` |
